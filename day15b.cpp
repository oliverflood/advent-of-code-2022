...
int main() {
	...
	for (int i = 0; i < sensors.size(); ++i)
	{
		for (int k = i; k < sensors.size(); ++k)
		{
			int sensor_distance = abs(sensors[i].first-sensors[k].first) + abs(sensors[i].second-sensors[k].second);
			if (sensor_distance-distances[i]-distances[k]-1 == 1) {
				cout << "horray" << endl;
				cout << "sensor 1: " << sensors[i].first << "  " << sensors[i].second << "  dist: " << distances[i] << endl;
				cout << "sensor 2: " << sensors[k].first << "  " << sensors[k].second << "  dist: " << distances[k] << endl;
			}
		}
	}

	// 13622251246513
	// calculated using desmos and point-slope form of two lines, will convert to code at some point maybe probably
