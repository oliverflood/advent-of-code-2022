...
pair<int, int> string_to_pair(string s) {
	...
	if (stoi(vec[1]) > largest_y)
		largest_y = stoi(vec[1]);
...
void place_sand(map<pair<int, int>, bool> &graph, pair<int, int> pos) {

	if (pos.second >= largest_y) {
		return;
	}
	...
	if (!graph[{pos.first, pos.second+1}]) {
		place_sand(graph, {pos.first, pos.second+1});
	}

	// down left
	if (!graph[{pos.first-1, pos.second+1}]) {
		place_sand(graph, {pos.first-1, pos.second+1});
	}
